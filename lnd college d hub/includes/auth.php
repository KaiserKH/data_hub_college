...
function requireLogin() {
    startSession();
    if (!isLoggedIn()) {
        if (!function_exists('buildEncryptedUrl')) {
            $encryption_file = __DIR__ . '/encryption.php';
            if (is_file($encryption_file)) {
        ...
        if (function_exists('buildEncryptedUrl')) {
            redirect(buildEncryptedUrl('/login.php', ['redirect' => $target]));
        }
...
function checkLoginAttempts($email) {
    // Login-rate restrictions are disabled in this clone.
    return true;
}
